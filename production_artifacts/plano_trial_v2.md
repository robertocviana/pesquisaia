# Technical Specification - Plano Trial e Limitações do Sistema (v2)

## Objetivo
Implementar um sistema de planos e assinaturas ("Trial" vs "Pro") na plataforma PesquisaIA. O plano default para novos cadastros será "Trial" (Gratuito). Limitações específicas serão aplicadas ao plano Trial para incentivar o upgrade para o plano "Pro", o qual será totalmente ilimitado para a criação e recebimento de respostas de pesquisas, mas com restrições uniformizadas de custos de IA. Uma funcionalidade de simulação/troca de planos será embutida na tela de Configurações para facilitar testes tanto pelo administrador quanto pelos usuários.

---

## 1. Regras de Limitações (Plano Trial vs Pro - v2)

A tabela abaixo resume as limitações atualizadas:

| Funcionalidade | Limite no Plano Trial | Plano Pro (Ilimitado/Premium) | Ação ao Atingir o Limite |
| :--- | :--- | :--- | :--- |
| **Pesquisas Criadas** | **Máximo de 3** pesquisas totais | Ilimitado | Bloqueia novas criações e duplicações (exibe banner de upgrade) |
| **Respostas por Pesquisa** | **Máximo de 10** respostas concluídas | Ilimitado | Encerra a pesquisa automaticamente ao atingir 10 respostas; impede novas respostas de participantes |
| **Exportação de Dados** | **Bloqueada** (CSV/PDF) | Liberado | Exibe mensagem de erro flash no painel com link de upgrade |
| **Relatórios de IA** | **1 geração** por pesquisa (Sem regenerar) | **1 geração** por pesquisa (Sem regenerar) | Oculta/bloqueia o botão de regenerar relatório se o relatório já existir |
| **Gerador de Respostas (Testes)** | **Máximo de 10 respostas** com IA (Híbrido) | **Máximo de 10 respostas** com IA (Híbrido) | Validação no backend impedindo solicitações > 10 respondentes |
...
- **Limitação de Relatórios**:
  - No método `handleRelatorioGerar()`, se já existir um relatório salvo no banco para a pesquisa, impedir nova geração (lançar erro flash "Esta pesquisa já possui um relatório gerado. Não é permitido regenerar relatórios"). Essa regra agora vale **tanto para o plano Trial quanto para o Pro**.

### 5.3 `[MODIFY] app/Controllers/ResponseController.php`
- **Limitação do Gerador**: No método `handleGenerateResponses()`, validar a quantidade selecionada `$count`.
  - Para **ambos os planos** (Trial e Pro): limitar o valor máximo a `10` respondentes por requisição.
  - A estratégia `'hybrid'` (IA) é permitida para ambos.

### 5.4 `[MODIFY] app/Controllers/RespondentController.php`
- **Limitação do Respondente**: Ao carregar a página da pesquisa (`intro` ou `chat`), verificar se o criador da pesquisa (`user_id` associado) está no plano `trial` e se o número de respostas atuais (`response_count`) já atingiu 10.
  - Se sim, e o participante for novo ou não tiver concluído, bloquear o acesso à pesquisa exibindo uma tela amigável informando que a pesquisa atingiu o limite de respostas permitidas para o plano gratuito (Trial) do organizador.
- **Fechamento Automático**: Atualizar o método `checkAutoClose` na classe `Survey` para também monitorar o limite de 10 respostas caso o dono da pesquisa seja `trial`.
...
- Ocultar o botão "Regenerar Relatório" completamente se o relatório já tiver sido gerado uma vez (regra unificada para Trial e Pro).

### 6.4 `[MODIFY] app/Views/surveys/respostas.php`
- Atualizar o seletor de quantidade no gerador de respostas para limitar a opção máxima em 10 respondentes (tanto para Trial quanto para Pro).
...
4. **Limite de 10 Respostas**:
   - Criar uma pesquisa no plano Trial. Simular/gerar 10 respostas.
...
6. **IA Report / Response Generator**:
   - Testar o gerador de respostas para ver se limita a 10 respondentes com IA em ambos os planos.
   - Testar a impossibilidade de regenerar o relatório da IA (o botão deve desaparecer após a primeira geração para ambos).
